#Overview of scripts:  
ETFModels.py is a wrapper for TSCV that takes a time series of signals and calculates   positions and strategy performance.TSCV takes data set, output from ETFModels.py, and the   number of k-folds to output various training and test set metrics. These are dependent on   the cross validation function chosen. Test_framework.py and test_linearModel.py, is a   template for executing the backtest cross validation, based on the instructions in the   Readme file.  
